...
public class SongScanning : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        StartCoroutine(ScanAndContinue(delegate(List<SongInfo> songs) { allSongs = songs; }));
    }
    ...
    private IEnumerator ScanAndContinue(OnFinished onFinished)
    {
        ...
        {
            loadingImage.color -= new Color(0,
                0,
                0,
                Time.deltaTime);
            yield return null;
    ...
    private List<SongInfo> Sort(List<SongInfo> songs)
    {
        ...
        for (int i = 0; i < songs.Count; ++i)
        {
            if (!songByArtists.ContainsKey(songs[i].displayArtist))
            {
                ...
                songByArtists.Add(songs[i].displayArtist,
                    songs[i]);
            }
        }
    ...
    private SongInfo CreateSongInfo(DirectoryInfo folder)
    {
        ...
        for (int i = 0; i < lines.Length; ++i)
        {
            if (lines[i].StartsWith("name"))
                songInfo.name = lines[i].Split("="[0])[1].Trim();
            if (lines[i].StartsWith("artist"))
                songInfo.artist = lines[i].Split("="[0])[1].Trim();
        }
