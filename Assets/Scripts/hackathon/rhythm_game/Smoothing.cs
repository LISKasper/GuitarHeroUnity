...
{
    private double smoothBpm;
    private double smoothTick;
    private readonly double visualOffset;
    ...
    public double SmoothTick(double tick,
        uint resolution)
    {
        ...
        if (!double.IsNaN(ticksPassed) && smoothBpm > 0)
            smoothTick += ticksPassed;
